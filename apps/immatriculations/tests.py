...
class ImmatriculationBase(APITestCase):
    ...
    def _dossier(self, statut=StatutDossier.VALIDE, vin="VF1RFB00X67891234"):
...
class AttributionTests(ImmatriculationBase):
    def test_composer_numero_unitaire(self):
        self.assertEqual(_composer_numero(1)[0].split()[0], "AA")
        self.assertTrue(_composer_numero(1)[0].startswith("AA 0001"))
        self.assertTrue(_composer_numero(9999)[0].startswith("AA 9999"))
        self.assertTrue(_composer_numero(10000)[0].startswith("AB 0001"))  # rollover de série

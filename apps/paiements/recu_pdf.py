"""Reçu officiel de paiement (PDF vectoriel ReportLab, avec QR de vérification)."""
...
def rendre_recu_pdf(paiement) -> bytes:
    d = paiement.dossier
    buf = BytesIO()
    c = canvas.Canvas(buf, pagesize=A4)
    c.setTitle(f"Reçu {paiement.reference}")
    ...
    c.drawString(M + 84, _y(M + 52), "Reçu de paiement")
    c.setFillColor(HexColor("#b9cbe6")); c.setFont("Helvetica", 9)
    c.drawString(M + 84, _y(M + 70), "Taxe d'immatriculation de véhicule")
    # Sceau PAYÉ
    c.setFillColor(VERT)
    c.roundRect(W - M - 92, _y(M + 58), 74, 26, 13, fill=1, stroke=0)
    c.setFillColor(BLANC); c.setFont("Helvetica-Bold", 12)
    c.drawCentredString(W - M - 55, _y(M + 41), "PAYÉ")
    ...
    c.drawString(M, _y(y + 30), _fmt(paiement.montant_total, paiement.devise))

    # QR (droite) — ancré sous l'en-tête pour ne pas chevaucher le sceau
    payload = f"SNICV|RECU|{paiement.reference}|{paiement.montant_total} {paiement.devise}|{paiement.reference_transaction}"
    ...
    c.drawCentredString(W - M - qs / 2, _y(y + qs + 12), "Vérifier l'authenticité")

    # Détail des lignes (sous le QR)
    y += qs + 30
    ...
    y += 26
    for ligne in paiement.detail:
        c.setFillColor(GRIS); c.setFont("Helvetica", 10)
        c.drawString(M + 4, _y(y), str(ligne.get("libelle", "")))
        c.setFillColor(ENCRE); c.setFont("Helvetica", 10)
        c.drawRightString(W - M - 4, _y(y), _fmt(int(ligne.get("montant", 0)), paiement.devise))
        y += 17
    c.setStrokeColor(BORD); c.setLineWidth(0.8)
    c.line(M, _y(y - 4), W - M, _y(y - 4))
    c.setFillColor(BLEU); c.setFont("Helvetica-Bold", 11)
    c.drawString(M + 4, _y(y + 12), "Total")
    c.drawRightString(W - M - 4, _y(y + 12), _fmt(paiement.montant_total, paiement.devise))
    y += 40

    # Informations
    infos = [
        ("Reçu N°", paiement.reference),
        ("Dossier", d.numero_dossier),
        ("Opérateur", paiement.operateur),
        ("Numéro", paiement.numero_telephone),
        ("Réf. transaction", paiement.reference_transaction or "—"),
        ("Date", paiement.paye_le.strftime("%d/%m/%Y %H:%M") if paiement.paye_le else "—"),
    ]
    ...
    c.drawString(M + 18, _y(by + 22), "PAIEMENT CONFIRMÉ")
    c.setFillColor(GRIS); c.setFont("Helvetica", 8.5)
    c.drawString(M + 18, _y(by + 38), "Ce reçu débloque l'émission du certificat d'immatriculation. Conservez-le.")
